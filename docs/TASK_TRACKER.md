...
| **===== Phase 2 - Mobility =====** |  |  |  |  |  |
| Wire brain <-> robot command interface (keep ACTION contract) | Phase 2 - Mobility | 🔵 In progress | P0 | Hardware integration | Interface should preserve action schema; software path starts with `MOTOR_ADAPTER_MODE=none|mock` while hardware backend is pending |
