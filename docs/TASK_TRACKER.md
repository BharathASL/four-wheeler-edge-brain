...
| Implement capture-time VAD with streaming audio capture | Phase 4.1 - Audio | 🟡 To do | P1 | None | Use webrtcvad library to detect voice during recording (not post-processing); stream input → VAD detection → buffer speech → end silence detection → send chunk to Vosk |
| ↳ Refactor SoundDeviceAudioAdapter to streaming mode | Phase 4.1 - Audio | 🟡 To do | P1 | Phase 4.1 parent | Replace fixed 2-second recording with streaming chunks (10-20ms); add VAD state machine (SILENCE → SPEECH → POST_SPEECH → SILENCE) |
| ↳ Add webrtcvad integration and configuration | Phase 4.1 - Audio | 🟡 To do | P1 | Phase 4.1 parent | Configure VAD aggressiveness level (0-3); support env controls for sensitivity tuning |
| ↳ Add buffering + adaptive padding logic | Phase 4.1 - Audio | 🟡 To do | P1 | Phase 4.1 parent | Buffer speech frames; continue recording for N ms after silence to capture tail; prevent fragmented speech chunks |
| ↳ Add tests for streaming VAD capture (unit + integration) | Phase 4.1 - Audio | 🟡 To do | P1 | Phase 4.1 parent | Test voice detection timing, silence padding, multiple utterances in one session |
