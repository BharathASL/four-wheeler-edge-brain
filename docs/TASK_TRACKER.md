...
| **===== Phase 2 - Mobility =====** |  |  |  |  |  |
| Wire brain <-> robot command interface (keep ACTION contract) | Phase 2 - Mobility | ✅ Done (Implemented) | P0 | None | ACTION contract preserved; runtime now supports software path with `MOTOR_ADAPTER_MODE=none|mock` while real hardware backend remains separate |
...
| Design audio pre-processing pipeline (VAD, noise gate, AGC) | Phase 4 - Audio | ✅ Done (Implemented) | P1 | None | Improve STT accuracy before decoder; Vosk `SetWords(True)` fix enables word-level confidence extraction; all stages tested and wired (`feature/phase4-audio-preprocessing-pipeline`) |
| ↳ Phase 1: Add preprocessing config fields + env bindings to config.py | Phase 4 - Audio | ✅ Done (Implemented) | P1 | None | 10 new fields with safe-bounds clamps; AUDIO_PREPROCESS_ENABLED=False by default |
| ↳ Phase 2+3: Implement AudioPreprocessor (noise gate, VAD, AGC) in audio_preprocessor.py | Phase 4 - Audio | ✅ Done (Implemented) | P1 | None | numpy-only; noise gate → VAD trim → AGC; stats counters; fail-open fallback |
| ↳ Phase 4: Wire AudioPreprocessor into SpeechInputListener (optional param) | Phase 4 - Audio | ✅ Done (Implemented) | P1 | None | preprocessor=None preserves existing behaviour; silence gate returns None without error code |
| ↳ Phase 5: Full test suite (test_audio_preprocessor.py + test_input_listener.py additions) | Phase 4 - Audio | ✅ Done (Implemented) | P1 | None | ~36 unit tests; no hardware deps; covers all stages, stats, fallback, listener wiring, and confidence extraction; 248 total tests passing |
| **Phase 4.1 - Audio: Real-time Voice Activity Detection (VAD)** |  |  |  |  |  |
| Implement capture-time VAD with streaming audio capture | Phase 4.1 - Audio | 🔵 In progress | P1 | None | Use webrtcvad library to detect voice during recording (not post-processing); stream input → VAD detection → buffer speech → end silence detection → send chunk to Vosk |
| ↳ Refactor SoundDeviceAudioAdapter to streaming mode | Phase 4.1 - Audio | ✅ Done | P1 | Phase 4.1 parent | Replace fixed 2-second recording with streaming chunks (10-20ms); add VAD state machine (SILENCE → SPEECH → POST_SPEECH → SILENCE) |
| ↳ Add webrtcvad integration and configuration | Phase 4.1 - Audio | ✅ Done | P1 | Phase 4.1 parent | Configure VAD aggressiveness level (0-3); support env controls for sensitivity tuning |
| ↳ Add buffering + adaptive padding logic | Phase 4.1 - Audio | ✅ Done | P1 | Phase 4.1 parent | Buffer speech frames; continue recording for N ms after silence to capture tail; prevent fragmented speech chunks |
| ↳ Add tests for streaming VAD capture (unit + integration) | Phase 4.1 - Audio | ✅ Done | P1 | Phase 4.1 parent | Test voice detection timing, silence padding, multiple utterances in one session |
| ↳ Compare noise gate + AGC (post-processing) vs capture-time VAD (streaming) | Phase 4.1 - Audio | 🟡 To do | P2 | Phase 4.1 parent | A/B test accuracy improvement: preprocessing alone vs streaming VAD alone vs both together |
